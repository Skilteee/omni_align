...
def smooth_fc_fc_temporary(fc1, fc2, scales, shifts=None):
    original_dtype = fc1.weight.dtype
    # only support for v_proj and out_pro now.
    ...
    fc1.temp_weight = fc1.temp_weight.to(original_dtype)
    fc1.temp_bias = fc1.temp_bias.to(original_dtype)

    fc2.temp_weight = fc2.weight * scales.view(1, -1)
    fc2.temp_weight = fc2.temp_weight.to(original_dtype)
    if hasattr(fc2, 'temp_bias') and fc2.temp_bias is not None:
        fc2.temp_bias = fc2.temp_bias.to(original_dtype)
...
def smooth_q_k_temporary(q_proj, k_proj, scales):
    original_dtype = q_proj.weight.dtype
    ...
    q_proj.temp_weight = q_proj.temp_weight.to(original_dtype)
    q_proj.temp_bias = q_proj.temp_bias.to(original_dtype)
    k_proj.temp_weight = k_proj.temp_weight.to(original_dtype)
    k_proj.temp_bias = k_proj.temp_bias.to(original_dtype)

def smooth_ln_fcs_temporary(ln, fcs, scales,shifts):
    original_dtype = ln.weight.dtype
    ...
    ln.temp_weight = (ln.weight / scales).to(original_dtype)
    ln.temp_bias = ln.temp_bias.to(original_dtype)

    if hasattr(ln, 'eps1'):
        ln.temp_eps1 = ((ln.eps1 / scales).to(original_dtype))
    ...
    for fc in fcs:
        ...
        fc.temp_weight = (fc.weight * scales.view(1,-1)).to(original_dtype)
        fc.temp_bias = fc.temp_bias.to(original_dtype)
...
def smooth_ln_fcs_inplace(ln, fcs, scales,shifts):
    ...
    if hasattr(ln, 'bias') and ln.bias is not None:
        ln.bias.sub_(shifts)
        ln.bias.div_(scales)
    if hasattr(ln, 'bias') and ln.bias is None:
        del ln.bias
    ...
    for fc in fcs:
        ...
        elif hasattr(fc, 'bias') and fc.bias is None:
            del fc.bias
...
def smooth_fc_fc_inplace(fc1, fc2, scales,shifts=None):
    ...
    elif hasattr(fc2, 'bias') and fc2.bias is None:
        del fc2.bias
