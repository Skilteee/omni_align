...
def process_harmful_dataset(dataset):

    processed_data = []
    for each in dataset:
        new_data = {"instruction": each["prompt"], "input": "", "output": each["response"]}
...
class InstructionDataset(Dataset):
    def __init__(self, dataset_config, tokenizer, partition="train", max_words=30, pad=True, harm_ratio=0.05):
        self.ann = json.load(open(dataset_config.data_path))

        dataset = load_dataset('PKU-Alignment/BeaverTails', data_dir='round0', split='train')

        dataset.filter(lambda x: x["is_safe"] == 0)
        dataset = dataset.select(range(int(len(self.ann) * harm_ratio)))
        dataset = process_harmful_dataset(dataset)
